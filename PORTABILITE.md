...
**État : ✅ réalisée** — `scripts/build-portable.mjs` + `scripts/check-portable.mjs` + `scripts/check-artifact.mjs` (raccourcis npm `build:portable` / `check:portable` / `check:artifact`, les deux premiers testés en CI, artefact gitignoré). Les étapes ci-dessous documentent la conception ; le README (§ Version portable) est la référence d'usage.
...
4. **Validation** : `npm run check:portable` (rebuild + vérifs statiques + test navigateur en `file://` — canvas, édition, persistance, zoom, autoimport) ; `npm run check` côté sources pour confirmer zéro régression. Les deux sont branchés en CI. **Note : Firefox n'est pas installé sur le poste de dev — le shim `localStorage` n'a pas été testé sur le navigateur cible** (mécanisme standard, Chrome/Edge/Safari inchangés).
   - **Validation de l'artefact publié** : `npm run check:artifact` (`scripts/check-artifact.mjs`) teste **l'artefact tel qu'il sort de la CI**, pas le fichier rebuildé localement — il télécharge le dernier `meshes-portable` du dernier run `master` réussi via `gh` CLI + `unzip` (repo dérivé du remote git, `--repo` pour surcharger) et lance le même parcours navigateur `file://` via le helper partagé `scripts/portable-browser-test.mjs` (dédoublonné avec `check-portable.mjs`). `npm run check:artifact -- --local <fichier>` (ou l'argument positionnel `<fichier.html>`) teste un fichier portable déjà sur disque — pratique pour re-tester un artefact téléchargé manuellement, sans prérequis `gh`/`unzip`. `--out-dir <dir>` contrôle le dossier d'extraction (défaut : temp, nettoyé ; jamais supprimé s'il préexistait), `--keep` le conserve. Ce test post-CI dépend d'un run `master` publié : il est **volontairement hors de `check:portable` et du workflow** — c'est un outil de vérification manuelle.
