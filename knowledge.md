...
- **Test:** `npm run check` — `node --check` on all `.js`/`.mjs` + the 11 browser smoke suites (auto-starts/stops the dev server). See « Dev tooling & CI » below.
- **Lint:** none.
...
| `npm run smoke[:suite]` | 11 headless browser suites (preview, edit, import, gestures, modals, rotate, circle, shapes, color, multipoint, palette) via `playwright-core` + harness `scripts/smoke_lib.mjs` (`CHROMIUM_PATH`, default `/usr/bin/chromium`). |
