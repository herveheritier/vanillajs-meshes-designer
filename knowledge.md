...
- **Test:** `npm run check` — `node --check` on all `.js`/`.mjs` + the 10 browser smoke suites (auto-starts/stops the dev server). See « Dev tooling & CI » below.
- **Lint:** none.
...
| Script | What it does |
|---|---|
| `npm run check` | `bash scripts/check.sh` : `node --check` on all `.js`/`.mjs` (incl. `scripts/`) then the 9 smoke suites (`npm run smoke`) — starts/stops the dev server itself if port 8000 is free. |
| `npm run smoke[:suite]` | 10 headless browser suites (preview, edit, import, gestures, modals, rotate, circle, shapes, color, multipoint) via `playwright-core` + harness `scripts/smoke_lib.mjs` (`CHROMIUM_PATH`, default `/usr/bin/chromium`). |
