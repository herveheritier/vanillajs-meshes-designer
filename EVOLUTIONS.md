...
- proposer un commentaire dans le hud à chaque action réalisée pour aider l'utilisateur
...
- ajouter un mode de sélection d'objet basé sur l'affichage de chaque objet sur un plan différent avec simulation d'affichage 3D des plan ; chaque plan est incliné autour d'un axe vertical virtuel pour donner à l'utilisateur une impression de décalage de chaque plan ; la position du pointeur sur l'horizontal fait varier l'inclinaison des plans et met en valeur un plan à la fois permettant de le sélectionner ; l'analogie la plus proche est la représentation sous la forme d'un kiosque de sélection
## évolutions effectuées sur l'application (date, id de commit)
- 06/08/2026 (7d1408c) : boutons pour forcer l'alignement et la répartition des points sélectionnés — panneau du bouton #align (groupe sélection) avec 4 actions sur la sélection de la forme active : ALIGNER X / Y (Alt+Flèche Gauche / Droite) alignent tous les points sur la coordonnée X / Y du PREMIER point sélectionné (l'ancre de référence, l'autre coordonnée conservée — convention des éditeurs vectoriels), RÉPARTIR X / Y (Alt+Shift+Flèche Gauche / Droite) espacent uniformément les points selon l'axe entre les deux extrêmes (qui restent en place, pas = (max-min)/(n-1)) ; chaque action = une entry undo UNIQUE (replaceShapePatch before/after, seules les coordonnées changent — les tris restent valides, les fills survivent) ; actions grisées selon la taille de la sélection (align ≥ 2 points, répartir ≥ 3) par updateAlignPanelButtons (sync par updateSelectionHud), bouton #align en ring inset quand le panneau est ouvert (updateAlignButton) ; le panneau reste ouvert après une action pour enchaîner (aligner X puis Y), fermeture par Echap / clic extérieur / re-clic ; raccourcis Alt+←/→ / Alt+Shift+←/→ (gardes typing / preview / AltGr / !e.repeat) ; README + knowledge.md + DESIGN.md §7.14 + aide mis à jour ; suite smoke smoke-align.mjs (15e suite)
- 06/08/2026 (94e8124) : boutons pour gérer l'ordre des formes — la forme active monte / descend d'un plan dans l'ordre du tableau (forme n = plan n dans la vue plans, cf. évolution prévisualisation : la forme d'indice le plus haut recouvre les précédentes) ; MONTER = indice +1 (1/3 → 2/3, la forme passe AU-DESSUS), DESCENDRE = indice -1 ; 2 boutons dans le groupe de navigation des formes (#moveShapeUp / #moveShapeDown, grisés aux bornes — 1re forme : descendre impossible, dernière : monter impossible — par updateShapeHud) + raccourcis Alt+Flèche Haut / Bas (mêmes fonctions, gardes typing / preview / AltGr / !e.repeat) ; une entry undo chacun (nouveau patch shapeMove {from,to} dans history.js — 16 B, la forme active suit le déplacement via activeShapeIndexPatch accolé, undo/redo restaurent ordre ET index actif) ; README + knowledge.md + DESIGN.md §7.13 + aide mis à jour ; suite smoke smoke-shapeorder.mjs (14e suite)
- 06/08/2026 (a216756) : le bouton prévisualiser évolue en CYCLE DE 3 ÉTATS (bouton ou P) : 1er clic = preview simple (comportement historique : chrome masquée, géométrie seule — la forme active remplie, les autres en contours atténués), 2e clic = PLANS — toutes les formes rendues COMME des plans remplis dans l'ORDRE du tableau (forme n = plan n : la forme d'indice le plus haut recouvre les précédentes là où elles se chevauchent, fills conservés) —, 3e clic = sortie ; en preview, la toolbar est réduite au seul bouton œil flottant (état actif vert preview / ambre + ring inset + libellé « plans » quand le 2e état est actif) pour que le cycle au clic soit possible ; Échap, clic gauche sur le canvas et Ctrl+S sortent DIRECTEMENT de l'un ou l'autre état (exitPreview, pas togglePreview) ; state.previewPlans (sous-état non persisté comme previewMode) ; README + knowledge.md + DESIGN.md §2.6 + aide mis à jour ; suite smoke smoke-preview.mjs couvre le cycle complet (3 clics, P, Échap, clic gauche, zoom molette dans les deux états)
- 06/08/2026 (6712179) : couper, copier, coller les éléments sélectionnés — presse-papiers INTERNE à l'application (pas navigator.clipboard : format interne {pointList, tris} non transportable par le presse-papiers système et accès fragile sous file://), contenu = points sélectionnés de la forme active + triangles ENTIÈREMENT contenus (fills conservés, ré-indexés en relatif pour le collage) ; 3 boutons dans le groupe sélection (#copy / #cut / #paste, grisés sans sélection / sans contenu via updateClipboardButtons, sync par updateSelectionHud) + raccourcis Ctrl+C / Ctrl+X / Ctrl+V (mêmes fonctions, garde typing pour ne pas casser le collage natif dans les modales, ignorés en preview) ; le collage cible TOUJOURS la forme active, décalé d'un demi-pas de grille par collage (cumul via clipboard.offset : les copies successives se cascadent), la copie collée devient la sélection ; couper = copier + deleteSelectedPoint (une seule sémantique de suppression), couper et coller = une entry undo chacun (replaceShapePatch) ; presse-papiers non persisté (session seule) ; README + knowledge.md + DESIGN.md §7.12 + aide mis à jour ; suite smoke smoke-clipboard.mjs (13e suite)
- 06/08/2026 (1fbd94b) : enregistrement de la scène par fenêtre de sélection de l'emplacement : cliquer SAVE (ou Ctrl+S) ouvre une fenêtre listant les scènes déjà sauvegardées (du plus récent au plus ancien, clé `meshesDesigner.savedScenes`, 20 max) avec un champ de RENOMMAGE pré-rempli (texte sélectionné, Entrée valide) ; la fenêtre se positionne sur l'emplacement PRÉCÉDENT (le plus récent, rangée « précédent » en surbrillance) ; valider télécharge `<nom>.json`, fait adopter ce nom à la scène (HUD + wire format) et mémorise l'emplacement ; Echap / Annuler / clic backdrop ferment sans sauvegarder ; en preview, Ctrl+S sort d'abord de la preview pour que la fenêtre soit visible ; suite smoke smoke-edit.mjs (section 4) + smoke-modals.mjs (section E)
- 06/08/2026 (bc628f8) : le bouton de fusion est VERROUILLABLE quand sa 2ème fonction est activée : un clic supplémentaire (avec 1 point sélectionné) verrouille le mode — un cadenas est dessiné sur le bouton (icône + ring inset vert) et le mode reste armé après chaque fusion réussie pour ENCHAÎNER plusieurs fusions sans réarmer ; un autre clic le désarme (mode + verrou), et si plus d'un point est sélectionné, le mode est déverrouillé ET désarmé (garde existante). Cycle du bouton en 3 états : armé → verrouillé → désarmé. Le verrou est transitoire (jamais persisté, comme le mode armé)
